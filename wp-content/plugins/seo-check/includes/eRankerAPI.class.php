...
if (!class_exists("eRankerAPI")) {
    ...
    class eRankerAPI {
         ...
         * An Error Object can be returned if the code is invalid. 
         */
        public function factors($language = 'en', $version = null) {
            if (empty($language)) {
                $language = 'en';
            }
			if(empty($version)){
            ...
            $ret = $this->download($this->apiurl . '/factor/list.json?' . http_build_query(array('language' => $language, 'email' => $this->email, 'apikey' => $this->apikey,'version' => $version)));
            if (!empty($ret['content'])) {
                $responseobj = json_decode(trim($ret['content']));
                return $responseobj;
            }
