...
function seocheck_act_report() {
    ...
    if (isset($_REQUEST['sc_url']) && !empty($_REQUEST['sc_url'])) {
        ...
        if (!empty($urlViewReporteRanker)) {
			//do not uncomment this: add redirect two times 
            ////add_action('wp_footer', 'seocheck_echo_redirect', 100);
			seocheck_echo_redirect();
            return;
        }
    }
    ...
    $language = (!empty($seocheck_settings["report_language"])) ? $seocheck_settings["report_language"] : "en";
    ...
    if (!empty($seocheck_report)) {
        ...
        if (empty($seocheck_factors) || isset($seocheck_factors->debug) || $seocheck_nocache) {
            $seocheck_factors = $erapi->factors($language,SEOCHECK_VER);
        ...
        if (isset($_GET['ajax']) && !empty($_GET['ajax']) && isset($_GET['factors']) && !empty($_GET['factors'])) {
			...
			//get thumbnail
			$thumbnail = '';
			if(isset($seocheck_report_scores) && gettype($seocheck_report_scores) === "object" && isset($seocheck_report_scores->thumbnail) && gettype($seocheck_report_scores->thumbnail) === "object"){
				if(!empty($seocheck_report_scores->thumbnail->data)){
					$thumbnail = $seocheck_report_scores->thumbnail->data;
				}
			}else if(isset($seocheck_report_scores) && gettype($seocheck_report_scores) === "object"  && isset($seocheck_report_scores->thumbnail) && gettype($seocheck_report_scores->thumbnail) === "array"){
				if(!empty($seocheck_report_scores->thumbnail['data'])){
					$thumbnail = $seocheck_report_scores->thumbnail['data'];
				}
			}
			
            $ajaxObj = eRankerCommons::ajaxReport($seocheck_report, $seocheck_factors, $seocheck_report_scores, $_GET['factors'], true, $thumbnail);
