...
function sc_audio($atts, $content = null) {
	...
	return '<audio' . ($id ? ' id="' . esc_attr($id) . '"' : '') . (!empty($class) ? ' class="'.esc_attr($class).'"' : '')
...
function sc_banner($atts, $content=null){	
	...
	return empty($src) ? '' : ('<a href="' . (empty($link) ? '#' : $link) . '"' . ($id ? ' id="' . esc_attr($id) . '"' : '')
...
function sc_br($atts, $content = null) {
	...
	return '<br' . (in_array($clear, array('left', 'right', 'both')) ? ' style="clear:' . esc_attr($clear) . '"' : '') . ' />';
}
// ---------------------------------- [/trx_br] ---------------------------------------
...
function sc_button($atts, $content=null){	
	...
	return '<div class="sc_button sc_button_style_' . esc_attr($style) . ' sc_button_size_' . esc_attr($size) . ($align && $align!='none' ? ' align'.esc_attr($align) : '') . (!empty($class) ? ' '.esc_attr($class) : '')
...
function sc_chat($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_chat' . (!empty($class) ? ' '.esc_attr($class) : '') . '"' . ($s ? ' style="'.esc_attr($s).'"' : '') . '>'
		. balanceTags($content)
...
function sc_columns($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="columnsWrap sc_columns sc_columns_count_' . esc_attr($count) . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>' . do_shortcode($content).'</div>';
}
...
function sc_contact_form($atts, $content = null) {
	...
	$THEMEREX_sc_contact_form_counter = 0;
	$content = do_shortcode($content);
	return '<div ' . ($id ? ' id="' . esc_attr($id) . '"' : '') . 'class="sc_contact_form sc_contact_form_'.($content ? 'custom' : 'contact').(!empty($class) ? ' '.esc_attr($class) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '') .'>'
...
function sc_dropcaps($atts, $content=null){
	...
	$content = do_shortcode($content);
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_dropcaps sc_dropcaps_style_' . esc_attr($style) . (!empty($class) ? ' '.esc_attr($class) : '') . '">'
...
function sc_emailer($atts, $content = null) {
	...
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="sc_emailer inputSubmitAnimation' . ($align && $align!='none' ? ' sc_align' . esc_attr($align) : '') . (sc_param_is_on($open) ? ' sFocus rad4 opened' : ' radCircle') . (!empty($class) ? ' '.esc_attr($class) : '') . '"' . ($s ? ' style="'.esc_attr($s).'"' : '') . '>'
...
function sc_google_map($atts, $content = null) {
	...
	return '<div id="' . esc_attr($id) . '" class="sc_googlemap'. (!empty($class) ? ' '.esc_attr($class) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '')
...
function sc_highlight($atts, $content=null){	
	...
	$s = ($color != '' ? 'color:' . esc_attr($color) . ';' : '')
		.($backcolor != '' ? 'background-color:' . esc_attr($backcolor) . ';' : '')
		.($size != '' ? 'font-size:' . getStyleValue($size) . ';' : '')
        .($line_height != '' ? 'line-height:' . getStyleValue($line_height) . ';' : '')
		.($style != '' ? $style : '');
	return '<span' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_highlight'.($type>0 ? ' sc_highlight_style_'.esc_attr($type) : ''). (!empty($class) ? ' '.esc_attr($class) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>' . do_shortcode($content) . '</span>';
}
// ---------------------------------- [/trx_highlight] ---------------------------------------
...
function sc_infobox($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_infobox sc_infobox_style_' . esc_attr($style) . (sc_param_is_on($closeable) ? ' sc_infobox_closeable' : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
			. do_shortcode($content) 
			. '</div>';
}
...
function sc_line($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_line' . ($style != '' ? ' sc_line_style_' . esc_attr($style) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'></div>';
}
...
function sc_list($atts, $content=null){	
	...
	return '<' . ($style=='ol' ? 'ol' : 'ul') . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_list sc_list_style_' . esc_attr($style) . (!empty($class) ? ' '.esc_attr($class) : '') . '"' . ($s!='' ? ' style="'.esc_attr($s).'"' : '') . '>'
			. do_shortcode($content) 
			. '</' .($style=='ol' ? 'ol' : 'ul') . '>';
}
...
function sc_popup($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_popup sc_popup_light mfp-with-anim mfp-hide' . ($class ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
			. do_shortcode($content) 
			. '</div>';
}
// ---------------------------------- [/trx_popup] ---------------------------------------
...
function sc_price_table($atts, $content=null){	
	...
	$THEMEREX_sc_price_table_counter = 0;
	$count = max(1, $count);
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_pricing_table columns_' . esc_attr($count) . ($align && $align!='none' ? ' align'.themerex_strtoproper($align) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"' . ($s!='' ? ' style="'.esc_attr($s).'"' : '') . '>'
			. do_shortcode($content)
		. '</div>';
}
...
function sc_price_item($atts, $content=null) {
	...
	return '<div class="sc_pricing_columns sc_pricing_column_'.esc_attr($THEMEREX_sc_price_table_counter). (!empty($class) ? ' '.esc_attr($class) : '').'"><ul'.(sc_param_is_on($animation) ? ' class="columnsAnimate"' : '') . ($id ? ' id="' . esc_attr($id) . '"' : '') . '>'
		. do_shortcode($content) 
		. '</ul></div>';
}
...
function sc_price_data($atts, $content=null) {
	...
	if ($type=='price' && $money!='') {
		...
		$content = do_shortcode($content);
	return '<li' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_pricing_data' . ($type!='' ? ' sc_pricing_'.esc_attr($type) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '">' . balanceTags($content) . '</li>';
}
...
function sc_quote($atts, $content=null){	
	...
	$content = do_shortcode($content);
	if (themerex_substr($content, 0, 2)!='<p') $content = '<p>' . esc_html($content) . '</p>';
	return '<blockquote' . ($id ? ' id="' . esc_attr($id) . '"' : '') . esc_html($cite_param) . ' class="sc_quote'. (!empty($class) ? ' '.esc_attr($class) : '').'"' . ($s!='' ? ' style="'.esc_attr($s).'"' : '') . '>'
		. balanceTags($content)
...
function sc_slider_item($atts, $content=null) {
	...
	return empty($src) ? '' : ('<li' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="' . esc_attr($THEMEREX_sc_slider_engine).'-slide' . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
...
function sc_table($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_table sc_table_style_' . max(1, min(4, $style)) . ' sc_table_size_' . esc_attr($size) . (!empty($align) ? ' sc_table_align_'.esc_attr($align) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '') .'>'
			. do_shortcode($content) 
			. '</div>';
}
...
function sc_tab($atts, $content = null) {
	...
	return '<div id="' . esc_attr($id) . '" class="sc_tabs_content' . ($THEMEREX_sc_tab_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_tab_counter == 1 ? ' first' : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '">'
...
function sc_team($atts, $content=null){	
	...
	$THEMEREX_sc_team_counter = 0;
	$content = do_shortcode($content);
	return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_team'.(!empty($class) ? ' '.esc_attr($class) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '') .'>'
...
function sc_team_item($atts, $content=null) {
	...
	if (!empty($name) && !empty($position)) {
		return '<div class="columns1_'.esc_attr($THEMEREX_sc_team_count) . (!empty($class) ? ' '.esc_attr($class) : '').'">'
...
function sc_testimonials($atts, $content=null){	
	...
	return ($title && $style>1 ? '<h2 class="sc_testimonials_title">'.esc_html($title).'</h2>' : '')
...
function sc_testimonials_item($atts, $content=null){
	...
	return '<li' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_testimonials_item swiper-slide'. (!empty($class) ? ' '.esc_attr($class) : '').'" style="'.(!empty($THEMEREX_sc_testimonials_width) ? 'width:' . esc_attr($THEMEREX_sc_testimonials_width) . ';' : '').(!empty($THEMEREX_sc_testimonials_height) ? 'height:' . esc_attr($THEMEREX_sc_testimonials_height) . ';' : '').'">'
...
function sc_tooltip($atts, $content=null){	
	...
	return '<span' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_tooltip_parent'. (!empty($class) ? ' '.esc_attr($class) : '').'">' . do_shortcode($content) . '<span class="sc_tooltip">' . esc_html($title) . '</span></span>';
}
// ---------------------------------- [/trx_tooltip] ---------------------------------------
...
function sc_zoom($atts, $content=null){	
	...
	return empty($src) ? '' : ((!sc_param_is_off($border) ? '<div class="sc_border sc_border_'.esc_attr($border).'">' : '')
